...
/**
 * Called when the user rejects the use of the microphone
 * @param  error The error
 */
Microphone.prototype.onPermissionRejected = function() {
  console.log('Microphone.onPermissionRejected()');
  this.requestedAccess = false;
  this.onError('Permission to access the microphone rejected.');
};
...
function SpeechRecognizer(_options) {
  ...
  this.mic.onAudio = function(data) {
    ...
    if (self.socket.connected)
      self.socket.emit('speech', {audio: data, rate: self.mic.sampleRate});
  };
